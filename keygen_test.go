...
func TestLicenseFile(t *testing.T) {
	...
	if now.Before(dataset.Issued) {
		t.Fatalf("System clock out of sync: t=%s", dataset.Issued)
	}

	if now.After(dataset.Expiry) {
		t.Fatalf("License file is expired: t=%s", dataset.Expiry)
	}
...
func TestMachineFile(t *testing.T) {
	...
	if now.After(dataset.Expiry) {
		t.Fatalf("Machine file is expired: t=%s", dataset.Expiry)
	}
